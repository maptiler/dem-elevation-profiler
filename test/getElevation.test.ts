...
import getElevation from '../src/getElevation'
import sharp from 'sharp'

const pngTile = await sharp('test/fixtures/12-775-1538.png')
  .raw()
  .toBuffer()
...
describe('getElevation', () => {
  it('png: returns the elevation of a point', async () => {
    const coord: [lon: number, lat: number] = [-111.8408203125, 40.81380923056976]
  ...
  it('png: returns the elevation of a point with a custom elevation parser', async () => {
    ...
    const elevation = getElevation(coord, tile, tileSize, tileImage, (r, g, b, a) => {
      return r + g + b + a
    })
  ...
  it('webp: returns the elevation of a point', async () => {
    const coord: [lon: number, lat: number] = [-111.95068359375, 40.8304347933204]
